...
MQTTClient::MQTTClient(const QString& broker_address, QObject* parent)
    : QObject(parent), m_broker_address(broker_address) {
...
void MQTTClient::publish_order(const QString& order_id,
    ...
    try {
        ...
        order_obj["destination"] = destination;
        order_obj["pin"] = pin;
        ...
        // 발행
        std::string topic = "delivery/order/001";
...
void MQTTClient::process_message(const QString& topic, const QString& payload) {
    try {
        ...
        if (topic == "shop/menu") {
            ...
            QString order_id = topic.split("/")[2];
